...
public:                                                                                                                \
    ...
    /* Provide static and non-static. These static functions are helpful to                                            \
     * avoid using strings to reference a class name. */                                                               \
    /* This allows a compiler error to be generated if the class name gets                                             \
     * changed, or a non-meta class name is used. */                                                                   \
    static std::type_index s_getTypeIndex()                                                                            \
    {                                                                                                                  \
        return std::type_index(typeid(classname));                                                                     \
    }                                                                                                                  \
    static constexpr std::string s_getTypeName()                                                                       \
    {                                                                                                                  \
        return #classname;                                                                                             \
    }                                                                                                                  \
    static constexpr std::string s_getParentTypeName()                                                                 \
    {                                                                                                                  \
        return #pclassname;                                                                                            \
    }                                                                                                                  \
    static constexpr bool s_hasParent()                                                                                \
    {                                                                                                                  \
        return !std::is_same_v<MetaParentType, Meta::Impl::NoParent>;                                                  \
    }                                                                                                                  \
    /* This sillyness is becasue you cannot specify pure virtual static                                                \
     * functions */                                                                                                    \
    /* I want to be able to call these functions via a pointer to the base but                                         \
     * I also want them to be constexpr, hence the static duplication */                                               \
    std::type_index getTypeIndex() const override                                                                      \
    {                                                                                                                  \
        return s_getTypeIndex();                                                                                       \
    }                                                                                                                  \
    std::string getTypeName() const override                                                                           \
    {                                                                                                                  \
        return s_getTypeName();                                                                                        \
    }                                                                                                                  \
    std::string getParentTypeName() const override                                                                     \
    {                                                                                                                  \
        return s_getParentTypeName();                                                                                  \
    }

#define _IMPLEMENT_META_OBJECT(classname)                                                                              \
    static_assert(std::is_base_of_v<Meta::MetaObject, classname>,                                                      \
                  "Meta objects must subclass the Meta::MetaObject class!");                                           \
    static_assert((!classname::s_hasParent()) || ((std::is_base_of_v<classname::MetaParentType, classname>) &&         \
                                                  (std::is_base_of_v<Meta::MetaObject, classname::MetaParentType>)),   \
                  "Class must subclass its provided parent, and the parent class must "                                \
                  "also be a meta object!");                                                                           \
...
template <typename T, typename C> T member_value_deducer(T C::*);
template <auto MV> using member_value_type_t = decltype(member_value_deducer(MV));

template <typename T, typename C, typename... Args> T member_function_return_deducer(T (C::*)(Args...));
template <typename T, typename C, typename... Args> T member_function_return_deducer(T (C::*)(Args...) const);
template <auto MV> using member_function_return_type_t = decltype(member_function_return_deducer(MV));

template <typename... Args> struct is_const_member_function : std::false_type
{
};
...
template <typename Func> struct function_args_deducer
{
    static_assert(sizeof(Func) == 0, "Template parameter is not a member function pointer");
};
template <typename R, typename C, typename... Args> struct function_args_deducer<R (C::*)(Args...)>
{
    using return_type = R;
    using class_type = C;
    // Boy this is crap....
    ...
    template <bool B, template <bool, typename...> class Apply> using apply_to_2 = Apply<B, Args...>;
};
template <typename R, typename C, typename... Args> struct function_args_deducer<R (C::*)(Args...) const>
{
    using return_type = R;
    using class_type = C;
    // Boy this is crap....
...
template <typename T, typename C, typename func> struct is_member_setter_function_pointer : std::false_type
{
};
template <typename T, typename C> struct is_member_setter_function_pointer<T, C, void (C::*)(T)> : std::true_type
{
};

template <typename T, typename C, typename func> struct is_member_getter_function_pointer : std::false_type
{
};
template <typename T, typename C> struct is_member_getter_function_pointer<T, C, T (C::*)(void) const> : std::true_type
{
};
...
template <typename... Ts> std::string pack_as_type_names()
{
    std::string ret;
...
template <typename T>
T safeOperationToType(const std::string &operationName, const std::string &objName, const std::type_index &type,
                      const std::function<std::any()> &call)
{
...
// Call this function once at the start of the program to initialize all meta
// info from any loaded libraries This function is here to defer
// meta-initialization until after logging has been initialized and shared
...
// Get a pointer to a classes meta info
// Returns nullptr if the class was not found
template <typename T> const ClassMetaBase *getClassMeta()
{
...
{
public:
    Prop(const std::string &name, const std::string &className) : name(name), className(className)
    {
    }
...
// The class must be templated to differentiate between const and non-const
// functions
template <bool isConst> class MemberFunctionPropBase : public Prop
{
public:
    using MetaObjSignature = std::conditional_t<isConst, const MetaObject &, MetaObject &>;
    MemberFunctionPropBase(const std::string &name, const std::string &className) : Prop(name, className)
    {
    }
    ...
    template <typename T> T invokeAsType(MetaObjSignature obj, const std::vector<std::any> &args) const
    {
        return Impl::safeOperationToType<T>("invokeAsType", getName(), getTypeIndex(),
                                            [&obj, &args, this]() { return invoke(obj, args); });
    }

    void setDefaultArgs(const std::vector<std::any> &args)
    {
        defaultArgs = args;
    };
    // Assuming you won't invoke with default args when the function takes no
    // args...
    std::any invokeDefaultArgs(MetaObjSignature obj) const
    {
        ...
        {
            Log::Error().log("Unable to invoke with default args! Property was "
                             "never given default args!");
            assert(false && "Property was never given default args!");
        }

        return std::any();
    }
    template <typename T> T invokeDefaultArgsAsType(MetaObjSignature obj) const
    {
        return Impl::safeOperationToType<T>("invokeDefaultArgsAsType", getName(), getTypeIndex(),
                                            [&obj, this]() { return invokeDefaultArgs(obj); });
    }
...
public:
    ...
    using FunctionSignature =
        std::conditional_t<isConst, ReturnType (ClassType::*)(Args...) const, ReturnType (ClassType::*)(Args...)>;
    ...
    MemberFunctionProp(const std::string &name, const std::string &className, FunctionSignature func)
        : MemberFunctionPropBase<isConst>(name, className), func(func)
    {
    }

    std::any invoke(MemberFunctionPropBase<isConst>::MetaObjSignature obj,
                    const std::vector<std::any> &args) const override
    {
        ...
        if (obj.isOrIsDerivedFrom(std::type_index(typeid(ClassType))))
        {
            try
            {
                if constexpr (!std::is_same_v<InvokeReturnType, void>)
                    return invoke_impl<Args...>(static_cast<ClassTypeSignature>(obj), args,
        ...
        {
            Log::Error().log("Failed to invoke for prop! Property \"{}\" "
                             "belongs to \"{}\" but given \"{}\"!",
                             Prop::getName(), Prop::getClassName(), obj.getTypeName());
...
private:
    ...
    InvokeReturnType invoke_impl(ClassTypeSignature obj, const std::vector<std::any> &args,
...
{
public:
    MemberPropertyBase(const std::string &name, const std::string &className) : Prop(name, className), readOnly(false)
    {
    }
    ...
    void setDefault(const std::any &val)
    {
        defaultValue = val;
    }
    ...
    template <typename T> T getAsType(const MetaObject &obj) const
    {
        return Impl::safeOperationToType<T>("getAsType", getName(), getTypeIndex(),
                                            [&obj, this]() { return getAsAny(obj); });
    }
...
template <class ClassType, typename MemberType> class TemplateMemberPropertyBase : public MemberPropertyBase
{
public:
    ...
    std::any createDefaultAsAny() const override
    {
    ...
    std::any getAsAny(const MetaObject &obj) const override
    {
        ...
        {
            Log::Error().log("Failed to getAsAny for \"{}\"! Property does not "
                             "belong to given object!",
                             getName());
            assert(false && "Property does not belong to given object!");
        }
        return std::any();
    ...
    void setFromAny(MetaObject &obj, const std::any &val) const override
    {
        ...
        if (obj.isOrIsDerivedFrom(std::type_index(typeid(ClassType))))
        {
            ...
            {
                Log::Error().log("Failed to set property for \"{}\"! Given "
                                 "'any' is the wrong type!",
                                 getName());
        ...
        {
            Log::Error().log("Failed to setFromAny! Property \"{}\" belongs to "
                             "\"{}\" but given \"{}\"!",
                             Prop::getName(), Prop::getClassName(), obj.getTypeName());
            assert(false && "Property does not belong to given object!");
        }
...
namespace Impl
{
template <typename ClassType> class MetaInitializer;
}
...
class MemberProperty : public TemplateMemberPropertyBase<ClassType, MemberType>
{
    ...
    MemberProperty(const std::string &name, const std::string &className, MemberType ClassType::*ptr)
        : TemplateMemberPropertyBase<ClassType, MemberType>(name, className), member(ptr)
    {
    }
...
public:
    MemberType get(const ClassType &obj) const override
    {
        return obj.*member;
    };
    void set(ClassType &obj, MemberType val) const override
    {
        obj.*member = val;
    };
};
...
class MemberPropertyFunctional : public TemplateMemberPropertyBase<ClassType, MemberType>
{
    ...
    // Creation is limited to the initializer class
    MemberPropertyFunctional(const std::string &name, const std::string &className,
                             void (ClassType::*setter)(MemberType), MemberType (ClassType::*getter)(void) const)
        : TemplateMemberPropertyBase<ClassType, MemberType>(name, className), setter(setter), getter(getter)
    {
    }
...
template <typename PS> class PropSetter
{
protected:
    template <typename T> friend class Impl::MetaInitializer;

    PropSetter(Prop *prop) : prop(prop)
    {
    }
...
class FunctionPropSetter : public PropSetter<FunctionPropSetter<isConst, Args...>>
{
    ...
    FunctionPropSetter(MemberFunctionPropBase<isConst> *prop)
        : PropSetter<FunctionPropSetter<isConst, Args...>>(prop), fprop(prop)
    {
    }
...
template <typename PType> class MemberPropSetter : public PropSetter<MemberPropSetter<PType>>
{
    template <typename T> friend class Impl::MetaInitializer;

    MemberPropSetter(MemberPropertyBase *prop) : PropSetter<MemberPropSetter<PType>>(prop), mprop(prop)
    {
    }
...
// Use the global Meta:: functions to get a ClassMetaBase pointer for a
// registered class
...
public:
    ClassMetaBase(const std::string &name, const std::string &parentName)
        : name{name}, parentName{parentName}, props{}, nonConstFunctions{}, constFunctions{}, parent{nullptr}
    {
    }
    ...
    const ClassMetaBase *getParent() const
    {
        return parent;
    }
    ...
    template <typename T> T createAsType()
    {
        return Impl::safeOperationToType<T>("createAsType", getName(), getTypeIndex(),
                                            [this]() { return createAsAny(); });
    }

    template <typename T> T createDefaultAsType()
    {
        return Impl::safeOperationToType<T>("createDefaultAsType", getName(), getTypeIndex(),
                                            [this]() { return createDefaultAsAny(); });
    }
...
// Small class used to store some information that is otherwise type-erased from
// the base class
template <class C> class ClassMeta : public ClassMetaBase
{
public:
    using ClassType = C;

    ClassMeta(const std::string &name, const std::string &parentName) : ClassMetaBase(name, parentName)
    {
    }
...
{
public:
    MetaInitializer(const std::string &name, const std::string &parentName) : m_classPtr(nullptr)
    {
        ...
        ::Meta::Impl::addDelayParentInitialize([this]() {
            ...
            if (m_classPtr->getParentName() != "Meta::Impl::NoParent")
            {
                ...
                {
                    Log::Critical().log("Specified parent \"{}\", of class \"{}\" was not "
                                        "found in the meta repo!",
                                        m_classPtr->getParentName(), m_classPtr->getName());
        ...
        ::Meta::Impl::addDelayValidate([this]() {
            ...
            auto validateProp = [this](const auto *myProp, const auto *prop) {
                ...
                {
                    Log::Error().log("Class \"{}\" defined member property \"{}\" which was "
                                     "already defined in a parent class!",
                                     m_classPtr->getName(), myProp->getName());
                    Log::Error().log("Property overriding is not supported (yet?)!");
                    assert(false);
                }
            };

            m_classPtr->parent->forAllMemberProps(
                [this, &validateProp](const ::Meta::MemberPropertyBase *prop) -> ::Meta::ClassMetaBase::InvokeResult {
                    for (const auto *myProp : m_classPtr->props)
                        validateProp(myProp, prop);
                    return ::Meta::ClassMetaBase::InvokeResult::CONTINUE;
                });

            m_classPtr->parent->forAllConstMemberFuncs(
                [this, &validateProp](
                    const ::Meta::MemberConstFunctionPropBase *prop) -> ::Meta::ClassMetaBase::InvokeResult {
                    for (const auto *myProp : m_classPtr->constFunctions)
                        validateProp(myProp, prop);
                    return ::Meta::ClassMetaBase::InvokeResult::CONTINUE;
                });

            m_classPtr->parent->forAllNonConstMemberFuncs(
                [this, &validateProp](
                    const ::Meta::MemberNonConstFunctionPropBase *prop) -> ::Meta::ClassMetaBase::InvokeResult {
    ...
    auto addMember(const std::string &name)
    {
        // I find the static asserts provide better feedback on errors as
        // opposed to just having these as a requires clause
        static_assert(is_member_setter_function_pointer<std::remove_cvref_t<member_function_return_type_t<getter>>,
                                                        ClassType, decltype(setter)>::value,
                      "Setter must match this signature where T matches the return type "
                      "of the getter: void(T)");
        static_assert(is_member_getter_function_pointer<std::remove_cvref_t<member_function_return_type_t<getter>>,
                                                        ClassType, decltype(getter)>::value,
                      "Getter must match this signature where T matches the input type "
                      "to the setter: T(void) const");
        using V = std::remove_reference_t<std::remove_const_t<member_function_return_type_t<getter>>>;
        auto *prop = internalAddMemberProperty(
            new MemberPropertyFunctional<ClassType, V>{name, m_classPtr->getName(), setter, getter}, name);
        return MemberPropSetter<V>(prop);
    }
...
private:
    ...
    constexpr auto get_member_vec_ptr()
        requires(isConst)
    {
    ...
    template <typename Constructor, bool isConst, auto function> auto internalAddMemberFunc(const std::string &name)
    {
        ...
        auto foundFunc =
            std::find_if(vec.begin(), vec.end(), [&name](const auto *comp) { return comp->getName() == name; });
        if (foundFunc == vec.end())
        {
            auto *func = new Constructor{name, m_classPtr->getName(), function};
            if (!func)
            {
                Log::Critical().log("Unable to allocate memory for new "
                                    "function property \"{}\"!",
                                    name);
                assert(func);
                delete func;
            ...
            Log::Debug().log("Registered new const function property: \"{}\" "
                             "to class: \"{}\"",
                             func->getName(), m_classPtr->getName());
            vec.push_back(func);
            return func;
        }
        else
        {
            Log::Error().log("Failed to add function property! Property \"{}\" "
                             "already exists on class \"{}\"",
                             name, m_classPtr->getName());
    ...
    MemberPropertyBase *internalAddMemberProperty(MemberPropertyBase *prop, const std::string &name)
    {
        ...
        auto foundProp =
            std::find_if(m_classPtr->props.begin(), m_classPtr->props.end(),
                         [prop](const MemberPropertyBase *comp) { return comp->getName() == prop->getName(); });
        if (foundProp == m_classPtr->props.end())
        {
            Log::Debug().log("Registered new property: \"{}\" to class: \"{}\"", prop->getName(),
                             m_classPtr->getName());
        ...
        {
            Log::Error().log("Failed to add property! Property \"{}\" already "
                             "exists on class \"{}\"",
                             prop->getName(), m_classPtr->getName());
            assert(false && "Property already added for class");
            delete prop;
        }
