...
public:
    ExampleStructBase() : zero(0)
    {
    }
    ExampleStructBase(int i) : zero(i)
    {
    }
...
{
    w.addMember<&ExampleStructBase::zero>("zero").setDescription("Test description zero!").setDefault(0);
}
...
public:
    ExampleStruct() : one(0), two(false), three(0)
    {
    }
    ExampleStruct(int i, bool b, float f) : ExampleStructBase(i * 2), one(i), two(b), three(f)
    {
    }
    ...
    void setThree(float val)
    {
        three = val;
    }
    float getThree() const
    {
        return three;
    }

    bool exampleRandomFunction(bool input)
    {
        return input;
    }
    bool exampleConstRandomFunction(bool input) const
    {
        return input;
    }
...
{
    w.addMember<&ExampleStruct::one>("one").setDescription("Test description one!").setDefault(80085);

    w.addMember<&ExampleStruct::two>("two").setDescription("Test description two!").setReadOnly();

    w.addMember<&ExampleStruct::setThree, &ExampleStruct::getThree>("three").setDescription("Test description three!");
...
int main()
{
    ...
    if (objMeta)
    {
        ...
        objMeta->forAllMemberProps([&obj](const Meta::MemberPropertyBase *prop) -> Meta::ClassMetaBase::InvokeResult {
            Log::Info().log("Property: Name = {}, Value = {}", prop->getName(),
                            Converter::getStringFromAny(prop->getTypeIndex(), prop->getAsAny(obj)));
        ...
        if (prop)
            Log::Info().log("Get property by name: {}",
                            Converter::getStringFromAny(prop->getTypeIndex(), prop->getAsAny(obj)));

        auto *func = objMeta->getNonConstFunc("randomFunction");
        if (func)
            Log::Info().log("Run non-const function: {}",
                            Converter::getStringFromAny(func->getTypeIndex(), func->invoke(obj, {false})));
