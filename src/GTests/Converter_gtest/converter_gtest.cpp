...
REGISTER_CONVERTER_FOR_TYPE(ExampleType, &ExampleType::toString, &ExampleType::fromString)
...
template <typename T> bool typeIsRegistered()
{
    auto itr = std::find_if(
        Converter::Impl::getRegisteredConverters().begin(), Converter::Impl::getRegisteredConverters().end(),
...
#define TEST_FLOAT_LIKE(type, suffix, max, min, funcName)                                                              \
    ...
    {                                                                                                                  \
        ...
        EXPECT_NEAR(Converter::getTypeFromString<type>(Converter::getStringFromAny<type>(std::any(test))), test,       \
                    nearEnough);                                                                                       \
        test = 10.0f;                                                                                                  \
        EXPECT_NEAR(Converter::getTypeFromString<type>(Converter::getStringForType(test)), test, nearEnough);          \
        EXPECT_NEAR(Converter::getTypeFromString<type>(Converter::getStringFromAny<type>(std::any(test))), test,       \
...
TEST_F(ConverterTest, Test_string)
{
    ...
    EXPECT_EQ(Converter::getTypeFromString<std::string>(Converter::getStringFromAny<std::string>(std::any(test))),
              test);
...
TEST_F(ConverterTest, UserDefinedType)
{
    ...
    EXPECT_EQ(Converter::getTypeFromString<ExampleType>(Converter::getStringFromAny<ExampleType>(std::any(test))),
              test);
...
TEST_F(ConverterTest, MinConvertersRegistered)
{
    ...
    ASSERT_GE(Converter::Impl::getRegisteredConverters().size(), numConverters)
        << "Some expected converters were not registered!";
    ASSERT_EQ(Converter::Impl::getRegisteredConverters().size(), numConverters)
        << "A default converter is not being accounted for in this test! Add "
           "it!!";
}
